/**
 * collision_mesh_generator.ts
 *
 * Scans reusable_assets for GLB files and generates .collision.bin sidecars.
 * Each .collision.bin contains the mesh's positions + indices (with world
 * transforms and auto-centering applied, matching the frontend GLB loader).
 * Large meshes (>50k triangles) are decimated using normal-aware clustering.
 *
 * Binary format (little-endian):
 *   [4 bytes] magic: 0x434F4C4C ("COLL")
 *   [4 bytes] version: 1
 *   [4 bytes] positionFloatCount (positions.length, NOT vertex count)
 *   [4 bytes] indexCount (indices.length)
 *   [positionFloatCount * 4 bytes] Float32 positions
 *   [indexCount * 4 bytes] Uint32 indices
 */

import fs from 'fs';
import path from 'path';

const MAGIC = 0x434F4C4C; // "COLL"
const VERSION = 1;
const DECIMATE_THRESHOLD = 50000; // triangles
const MAX_PHYSICS_TRIS = 200000;

// ── GLB Parsing (minimal — positions + indices only) ─────────────────────

function parseGLBCollision(buffer: Buffer): { positions: Float32Array; indices: Uint32Array } | null {
  ...
  // Collect positions and indices from all mesh primitives
  ...
  for (const nodeIdx of nodesToProcess) {
    ...
    for (const prim of meshDef.primitives) {
      ...
      const rawPos = getAccessorData(gltf, binData, posIdx);
  ...
  // Merge all position arrays
  let totalLen = 0;
  for (const a of allPositions) totalLen += a.length;
  const positions = new Float32Array(totalLen);
  let off = 0;
  for (const a of allPositions) { positions.set(a, off); off += a.length; }

  // Auto-center to bottom-center (matching frontend glb_loader.ts)
  ...
  return { positions, indices: new Uint32Array(allIndices) };
}

// ── Node world matrices (ported from frontend glb_loader.ts) ─────────────
...
function getAccessorData(gltf: any, binData: Buffer, accessorIdx: number): Float32Array | Uint16Array | Uint32Array {
  ...
  // Create an ArrayBuffer from the relevant slice of binData
  ...
  switch (accessor.componentType) {
    case 5126: return new Float32Array(ab, 0, totalElements); // FLOAT
    case 5123: { // UNSIGNED_SHORT
      const shortSlice = binData.subarray(byteOffset, byteOffset + totalElements * 2);
      const shortAb = shortSlice.buffer.slice(shortSlice.byteOffset, shortSlice.byteOffset + shortSlice.byteLength);
      return new Uint16Array(shortAb, 0, totalElements);
    }
    case 5125: return new Uint32Array(ab, 0, totalElements); // UNSIGNED_INT
...
// ── Decimation (ported from frontend editor_context.ts) ──────────────────
...
function decimateMesh(
  ...
  const computeCluster = (gridRes: number): number => {
    const cellX = (maxX - minX) / gridRes || 1;
    const cellY = (maxY - minY) / gridRes || 1;
    const cellZ = (maxZ - minZ) / gridRes || 1;
    const gr1 = gridRes + 1;
  ...
  const cellMap = new Map<number, { sumX: number; sumY: number; sumZ: number; count: number; newIdx: number }>();
  let newVertCount = 0;
  ...
  for (let i = 0; i < vertCount; i++) {
    ...
    if (!cell) { cell = { sumX: 0, sumY: 0, sumZ: 0, count: 0, newIdx: newVertCount++ }; cellMap.set(key, cell); }
    cell.sumX += x; cell.sumY += y; cell.sumZ += z; cell.count++;
    vertexRemap[i] = cell.newIdx;
  }

  const newPositions = new Float32Array(newVertCount * 3);
  for (const cell of cellMap.values()) {
    const idx = cell.newIdx * 3;
  ...
  return { positions: newPositions, indices: new Uint32Array(newIndicesArr) };
}

// ── Write .collision.bin ─────────────────────────────────────────────────

function writeCollisionBin(filePath: string, positions: Float32Array, indices: Uint32Array): void {
  const headerSize = 16;
  const posBytes = positions.length * 4;
  const idxBytes = indices.length * 4;
  const buf = Buffer.alloc(headerSize + posBytes + idxBytes);
  ...
  Buffer.from(positions.buffer, positions.byteOffset, posBytes).copy(buf, headerSize);
  Buffer.from(indices.buffer, indices.byteOffset, idxBytes).copy(buf, headerSize + posBytes);
...
export async function generateCollisionMeshes(assetsDir: string): Promise<void> {
  ...
  for (const glbPath of glbFiles) {
    const binPath = glbPath.replace(/\.glb$/i, '.collision.bin');
    ...
    try {
      ...
      const mesh = parseGLBCollision(buffer);
      ...
      writeCollisionBin(binPath, positions, indices);
      generated++;
    } catch {
      failed++;
    }
  ...
  console.log(`[CollisionMesh] ${total} GLB files: ${generated} generated, ${skipped} cached, ${failed} failed`);
}

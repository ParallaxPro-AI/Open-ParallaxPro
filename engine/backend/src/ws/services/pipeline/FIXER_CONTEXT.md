...
- `uvScaleX` / `uvScaleY`: numbers (default `1.0`). Tile-count along the U/V axes when sampling the texture. Primitive meshes (cube, plane, sphere, cylinder, cone) all get UVs spanning `(0,0)→(1,1)` across the whole surface, so a `textureBundle` on a 32m wall stretches the texture to 32m tall — looks blurry and wrong. Set `uvScaleX/Y` to tile it. **Rule of thumb**: tile count ≈ mesh-size-in-meters ÷ desired-texture-size-in-meters. Prototype textures are designed at ~1m per tile, so for a 1m-tile pattern on a wall scaled `[32, 3, 0.3]` use `uvScaleX: 32, uvScaleY: 3`. For ground planes scaled `[40, 1, 40]` use `uvScaleX: 40, uvScaleY: 40`. Skip when no texture is bound (color-only meshes don't need it). Don't apply to GLB custom meshes — those carry their own UVs from the artist. Common fix when a user reports "the texture looks blurry / smeared / wrong-size" on a primitive.
...
11. **Jagged roads / ugly terrain transitions**: Multiple overlapping plane entities used for ground with different textures → the edges are jagged and there's z-fighting. Fix: replace the stacked ground planes with a single `heightmapTerrain` block in `03_worlds.json` using `layers`, `paints`, and `paths` (see CREATOR_CONTEXT.md § "Terrain — heightmapTerrain"). Remove the old ground entity defs from `02_entities.json` and their placements from `03_worlds.json`. The terrain system blends textures smoothly with configurable feathered edges.

12. **"The terrain is flat — make it hilly / bumpy / mountainous"**: by default `heightmapTerrain` produces a flat surface (Y=0). Add an `elevation` block (sibling of `layers` / `paints`) with `noise` for rolling background hills, `hills` for explicit bumps/pits, and `flat_zones` to keep player spawn / building footprints walkable. The terrain entity gets a matching trimesh collider automatically — do not add a separate `ground_collider` plane (it conflicts with the bumpy surface). See CREATOR_CONTEXT.md § "Elevation — making terrain go up and down".

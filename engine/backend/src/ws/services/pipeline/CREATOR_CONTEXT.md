...
**Collider rule for `/assets/generated/*` — always `"collider": "mesh"`.** AI-generated GLBs have irregular silhouettes (cars with mirrors and spoilers, characters with outstretched limbs, props with thin spires). The default `box` shape would auto-fit to the loose AABB and add huge invisible collision volume around the empty space — players bump into nothing, projectiles stop in mid-air. Use the exact triangle hull instead:
```json
"physics": { "type": "static", "collider": "mesh" }
```
`validate.sh` emits a non-fatal `generated-glb-non-mesh-collider` warning for any `/assets/generated/*` entity authored with `box` / `sphere` / `capsule` — fix the JSON when you see it. Holds for static and kinematic bodies. Avoid generated GLBs for `dynamic` rigidbodies — Rapier's mesh shape isn't supported on dynamic bodies; if you need a moving generated prop, swap to a pack asset with a clean shape.
...
- `"collider": "mesh"` — exact triangle hull from the GLB. Slow; use for
  ...
  assets above; `validate.sh` warns when a generated path is authored with
  `box` / `sphere` / `capsule`). Never on dynamic bodies.

...
class HeroAbilitiesBehavior extends GameScript {
    ...
    onUpdate(dt) { this._qTimer-=dt; this._eTimer-=dt; this._rTimer-=dt;
        ...
        if(qFire){this._qTimer=this._q_cooldown;this._aoeAttack(this._q_damage,this._q_range);
        ...
        if(rFire){this._rTimer=this._r_cooldown;this._aoeAttack(this._r_damage,this._r_range);

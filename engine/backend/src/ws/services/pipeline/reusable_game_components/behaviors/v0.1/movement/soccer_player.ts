...
class SoccerPlayerBehavior extends GameScript {
    ...
    onUpdate(dt) {
        ...
        if (moving) {
            // Negate both atan2 args so the model faces the direction of
            // motion (engine Y-rotation is CCW-from-above; the unnegated
            // form points 180° opposite the velocity vector).

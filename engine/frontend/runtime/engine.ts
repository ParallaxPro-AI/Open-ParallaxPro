...
export class ParallaxEngine {
    ...
    setEditorMode(enabled: boolean): void {
        ...
        // the way of the editor's gizmos. Suspension is keyed so the
        // overlay correctly composes with other independent reasons
        // (e.g. AI Assistant chat sheet open).
        try { this.globalContext.mobileOverlay?.setSuspended(enabled, 'editor-mode'); } catch { /* swallow */ }
        if (enabled) {
            ...
            // Drop cached skinning bind groups. Joint buffers now carry
            // unique labels so the (idx, jointBuf.label) cache key
            // genuinely identifies each (slot, buffer) pair within a Play
            // session, but on Play→Stop→Play the AnimatorComponents are
            // recreated with fresh buffers and the old entries become
            // dangling. Drop the whole map so the next Play starts clean.
